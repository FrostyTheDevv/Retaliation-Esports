...
export default function PlayerEditForm({ player: initialPlayer }: PlayerEditFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    inGameName: initialPlayer.inGameName || "",
    role: initialPlayer.role || "",
    bio: initialPlayer.bio || "",
    image: initialPlayer.image || "",
    goals: initialPlayer.goals?.toString() || "",
    assists: initialPlayer.assists?.toString() || "",
    saves: initialPlayer.saves?.toString() || "",
    twitter: initialPlayer.twitter || "",
    twitch: initialPlayer.twitch || "",
    youtube: initialPlayer.youtube || "",
    instagram: initialPlayer.instagram || "",
    discord: initialPlayer.discord || "",
    steam: initialPlayer.steam || "",
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/rosters/${initialPlayer.rosterId}/players/${initialPlayer.id}`,
        {
          method: "PATCH",
          headers: {
      ...
      router.push(`/admin/rosters/${initialPlayer.rosterId}/edit`)
      router.refresh()
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Link
            href={`/admin/rosters/${initialPlayer.rosterId}/edit`}
          ...
          <div>
            ...
            <p className="text-gray-400 mt-1">
              Editing {initialPlayer.name} in {initialPlayer.roster.name}
            </p>
          </div>
        </div>
        <button
      ...
      {deleteConfirm && (
        <div className="bg-red-500/10 border border-red-500 rounded-lg p-6">
          <h3 className="text-red-500 font-bold text-lg mb-2">⚠️ Delete Player?</h3>
          <p className="text-gray-300 mb-4">
            This will permanently delete <strong>{initialPlayer.name}</strong>. This action cannot be undone.
          </p>
